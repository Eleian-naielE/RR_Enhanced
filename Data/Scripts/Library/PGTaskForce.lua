...
function FundBases(player, target)
	...
	BlockOnCommand(GiveDesireBonus(player, "Upgrade_Starbase", target, 15, 40)) 
	...
	BlockOnCommand(GiveDesireBonus(player, "Build_Initial_Groundbase_Only", target, 15, 40))
end

function Exit_Plan_With_Possible_Sleep(gtf, stf)
	difficulty = "Easy"
	...
	if stf and not LandSecured then
		stf.Move_To(FindTarget.Reachable_Target(PlayerObject, "One", "Friendly", "Friendly_Only", 1.0, AITarget))
	end
	
    if stf then
        stf.Release_Forces(1.0)
    end
	if gtf then
		gtf.Release_Forces(1.0)
	end
...
function Release_Excess_Spaceforce(tf, player, target)
	if EvaluatePerception("Is_Connected_to_Enemy", player, target) == 0 then
		DebugMessage("%s -- releasing excess forces", tostring(Script))
		tf.Release_Forces(0.5)
	end
end

function Release_Excess_Groundforce(tf)
	local force_count = tf.Get_Force_Count()
	
	if force_count > 10 then
		tf.Release_Forces(0.5)
	end
end
...
--	end
	...
	if Get_Game_Mode() == "Space" then
		...
		if tf_to_reinforce.Get_Force_Count() ~= 0 then
			...
			good_loc = FindTarget(tf_to_reinforce, "Good_Reinforce_Zone", "Tactical_Location", 0.9)
			
			if good_loc then
				-- Reinforce new units and anti-idle existing units waiting for the reinforce
				if BlockOnCommand(tf_to_reinforce.Reinforce(good_loc), lib_reinforce_timeout, Anti_Idle_Taskforce) then
		...
		if second_try_tf and second_try_tf.Get_Force_Count() ~= 0 then
			...
			good_loc = FindTarget(second_try_tf, "Good_Reinforce_Zone", "Tactical_Location", 0.9)
		...
		-- Try to land near a friendly capital ship, some structure, the target itself, or finally the start location.
		lib_base_list = Find_All_Objects_Of_Type(player, "Capital")
		if lib_base_list[1] then
			DebugMessage("%s-- Trying to reinforce by friendly capital ship", tostring(Script))
			BlockOnCommand(tf_to_reinforce.Reinforce(lib_base_list[1]), lib_reinforce_timeout)
		else
			lib_base_list = Find_All_Objects_Of_Type(player, "SpaceStructure")
			if lib_base_list[1] then
	...
	-- otherwise we'll reinforce for a long time and then the plan will fail because it may no longer be relevant.
		...
		if tf_to_reinforce.Are_All_Units_On_Free_Store() then
			...
			if AITarget then
				...
				friendly_loc = FindTarget(tf_to_reinforce, "Current_Friendly_Location", "Tactical_Location", 1.0)
...
function Release_To_Hide(tf, unit)
	...
	if not flee_loc then
		flee_loc = FindTarget(tf, "Space_Area_Is_Hidden", "Tactical_Location", 1.0, 500.0)
	end
	if flee_loc then
...
function GalacticAttackAllowed(difficulty, ai_territories_just_gained)
	...
	if num_ai_attacks_left > 0 then
		...
		if player_territories_gained >= player_gains_before_attacking then
			...
			DebugMessage("%s -- %d player territories gained, %d allowed before responding so pausing before next attack"
...
function DifficultyBasedMinPause(difficulty)
	...
	DebugMessage("%s-- difficulty based pause is %s seconds for difficulty %s", tostring(Script), tostring(difficulty), tostring(sleep_duration))
